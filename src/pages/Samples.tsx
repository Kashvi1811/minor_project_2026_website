...
const Samples = () => (
  <DocsLayout>
    <PageHeader title="Sample Programs" headingColor="text-pink-500" description="Learn by example — three programs demonstrating the VM's capabilities." />
    ...
    {programs.map((prog) => (
      <SectionCard key={prog.name} title={prog.name} className="mb-4">
        ...
        <div className="mt-3 inline-flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm font-mono font-semibold text-red-700 bg-slate-100">
          Result: {prog.result}
        </div>
      </SectionCard>
    ))}
